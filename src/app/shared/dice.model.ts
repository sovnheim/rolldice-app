...
export class DiceModel {
  ...
  getVerboseResult(): string {
    if (this.modifiers.length > 0) {
      return `${this.rolledSet.join('+')} (${DiceModel.formatModifiers(
        this.modifiers
  ...
  runBasicStats(): void {
    this.stats = {
      ...
      average:
        DiceModel.diceAverage(this.rollableSet) +
  ...
  calculateDistribution(rollableSet: number[]): void {
    ...
    const cartesian = (...x) =>
      x.reduce((a, b) => a.flatMap((d) => b.map((e) => [d, e].flat())));

    const output = cartesian(...generator).map((outcome) =>
